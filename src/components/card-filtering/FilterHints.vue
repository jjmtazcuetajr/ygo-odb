...
<template>
  <TooltipProvider v-if="isHoverDetected" :delay-duration="100" :disable-hoverable-content="true"
    ...
    <TooltipRoot>
      <TooltipTrigger aria-label="Hint"
        class="rounded-md size-[20px] flex items-center justify-center cursor-pointer hover:bg-neutral-200 dark:hover:bg-neutral-700">
  ...
  <PopoverRoot v-else>
    <PopoverTrigger aria-label="Hint"
      class="rounded-md size-[20px] flex items-center justify-center cursor-pointer active:bg-neutral-200 dark:active:bg-neutral-700">
      <CircleHelp class="size-[14px] sm:size-[16px]" />
    </PopoverTrigger>
    <PopoverPortal>
